...
export const OperatorTemplates: React.FC<{ onUseTemplate: (id: string) => void }> = ({ onUseTemplate }) => (
  <div className="space-y-4">
    ...
    <p className="text-xs text-zinc-500">Templates are pre-filled configurations, not an authority. Built through the same creation model; management exposes metadata members never see.</p></div>
      ...
      {OPERATOR_TEMPLATES.map((t) => (
        <div key={t.id} className="bg-white rounded-2xl border border-zinc-200 p-4 space-y-2">
          ...
          <div className="flex flex-wrap gap-1.5 pt-1">
            ...
            <button className="px-2.5 py-1.5 bg-zinc-100 text-[11px] font-bold rounded-lg cursor-pointer">Duplicate (mock)</button>
            <button className="px-2.5 py-1.5 bg-zinc-100 text-[11px] font-bold rounded-lg cursor-pointer">Publish (mock)</button>
            <button className="px-2.5 py-1.5 bg-zinc-100 text-[11px] font-bold rounded-lg cursor-pointer">Retire (mock)</button>
          </div>
        </div>
      ))}
    </div>
