  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <p className="text-xs text-zinc-500">Who is this user? Are they active? What access, groups, challenges? Mock suspend/reactivate with audit trail. Impersonation is <strong>not</strong> offered (prototype-only if ever).</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="bg-white rounded-2xl border border-zinc-200 p-4 sm:p-5 space-y-3 h-fit">
          ...
          <div className="flex gap-2">
            <button className="px-3 py-1.5 bg-white border border-zinc-200 text-xs font-bold rounded-lg cursor-pointer">Suspend (mock)</button>
            <button className="px-3 py-1.5 bg-white border border-zinc-200 text-xs font-bold rounded-lg cursor-pointer">Reactivate (mock)</button>
          ...
          <p className="text-[10px] text-zinc-400">Mock actions only — each writes to the Audit Log in this reference.</p>
        </div>
      </div>
    </div>
  );
