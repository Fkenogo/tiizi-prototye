  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6 sm:py-8 space-y-6">
      <div className="border-b border-zinc-200 pb-5">
        <p className="text-[11px] font-bold uppercase tracking-wider text-orange-600 flex items-center gap-1.5"><Sparkles className="w-3.5 h-3.5" /> Pre-filled configurations — not a separate authority</p>
          ...
          <div key={tpl.id} className="bg-white rounded-2xl border border-zinc-200 p-4 flex flex-col justify-between hover:border-orange-300 transition-colors">
            <div>
              <div className="flex items-center justify-between mb-1.5">
                <span className="text-[10px] font-bold uppercase text-orange-600">{tpl.type}</span>
      ...
      {preview && (
        <div className="bg-zinc-900 text-white rounded-2xl p-5 sm:p-6">
          ...
          <div className="mt-2 text-xs text-zinc-300">Type: <strong className="text-white">{preview.type}</strong> · Duration: <strong className="text-white">{preview.durationDays} days</strong> · Allowed edits before launch: targets, schedule, group, story.</div>
